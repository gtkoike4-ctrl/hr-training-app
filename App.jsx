  ...
  if(!role) return (
    <div style={{minHeight:"100vh",background:COLORS.bg,display:"flex",flexDirection:"column",alignItems:"center",justifyContent:"center",padding:24,fontFamily:"'Noto Sans JP',sans-serif"}}>
      ...
      <div style={{width:"100%",maxWidth:400,textAlign:"center"}}>
        ...
        <div style={{fontSize:22,fontWeight:800,color:COLORS.text,marginBottom:6}}>HR Training Platform</div>
        <div style={{fontSize:13,color:COLORS.textMuted,marginBottom:36}}>製造・メーカー系 新入社員教育システム</div>
        <div style={{display:"flex",flexDirection:"column",gap:12}}>
          <button onClick={()=>{setRole("employee");setTab("ホーム");}} style={{background:COLORS.accent,color:"#fff",border:"none",borderRadius:12,padding:"16px 0",fontWeight:800,fontSize:15,cursor:"pointer"}}>👤 新入社員としてログイン</button>
          <button onClick={()=>{setRole("admin");setTab("管理");}} style={{background:COLORS.surface,color:COLORS.text,border:`1px solid ${COLORS.border}`,borderRadius:12,padding:"16px 0",fontWeight:700,fontSize:15,cursor:"pointer"}}>🔧 管理者としてログイン</button>
        </div>
      </div>
    </div>
  );
